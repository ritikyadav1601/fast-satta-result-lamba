        ...
        <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
            ...
            @foreach ($users as $game )
                ...
                <tr class="odd:bg-white odd:dark:bg-gray-900 even:bg-gray-50 even:dark:bg-gray-800 border-b dark:border-gray-700">
                    ...
                    <td class="px-6 py-4">{{ $game->email }}</td>
               @if($game->id !== 1)
                    <td class="px-6 py-4">
                        <a href="{{ route('admin.user.edit', $game->id) }}" class="font-medium text-blue-600 dark:text-blue-500 hover:underline btn btn-primary">Edit</a>
                        <a href="{{ route('admin.user.delete', $game->id) }}" class="font-medium text-blue-600 dark:text-blue-500 hover:underline btn btn-danger">Delete</a>
                    </td>
                </tr>
                @endif
            @endforeach
    ...
    <div id="popup" class="fixed inset-0 flex items-center justify-center z-50 bg-gray-800 bg-opacity-50 hidden">
        <div class="bg-white dark:bg-gray-900 p-8 rounded-lg shadow-lg w-full max-w-md">
            ...
            <form method="POST" action="{{ route('admin.user.store') }}">
                ...
                <div class="mb-4">
                    <label for="name" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Email</label>
                    <input type="text" id="name" name="email" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm dark:bg-gray-700 dark:text-white">
                </div>

                <div class="mb-4">
                    <label for="name" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Password</label>
                    <input type="text" id="name" name="password" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm dark:bg-gray-700 dark:text-white">
                </div>
                @if($otherChart->count() > 0)
                    <div class="mb-4">
                        <label for="time" class="block text-sm font-medium text-gray-700 dark:text-gray-300"> Other Chart</label>
