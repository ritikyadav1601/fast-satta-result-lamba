...
export default function ExtraGamesResults({games,results}) {
  ...
  return <section className="tablebox1 extra-results-section" style={{margin:'0 0 5px'}}><div className="table-responsive"><table className="table table-bordered"><thead className="forblack"><tr><th>सट्टा का नाम</th><th>कल आया था</th><th>आज का रिज़ल्ट</th></tr></thead><tbody>{games.map(game=>{const href=`/chart/${gameSlug(game.name)}`;return <tr key={`${game.source||'extra'}-${game.id}`}><td className="foryellow"><Link className="gamenameeach" href={href}><strong>{game.name}</strong></Link><br/><strong>{showTime(game.resultTime)}</strong></td><td className="yesterday-number"><strong>{showResult(values.get(`${game.id}:${yesterday}`))}</strong></td><td className="today-number"><strong>{pendingResult(values.get(`${game.id}:${currentDate}`))}</strong></td></tr>})}</tbody></table></div></section>;
}
