...
const sections = {
  ...
  khaiwalCharts: { label: 'Khaiwal Charts', fields: [] },
};
...
export default function AdminApp({ initialSection = 'results' }) {
  ...
  async function saveKhaiwal(event,collection) {
    event.preventDefault();
    const form=event.currentTarget;
    const existing=collection==='otherCharts'?data.otherCharts?.[0]:null;
    const item={...(existing||{}),...Object.fromEntries(new FormData(form))};
    const response=await fetch('/api/admin/data',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({collection,item})});
    const saved=await response.json();
    if(!response.ok){setMessage(saved.error||'Could not save.');return;}
    if(collection==='khaiwal1')setData(current=>({...current,settings:{...current.settings,...saved}}));
    else setData(current=>({...current,otherCharts:[saved]}));
    setMessage(`${collection==='khaiwal1'?'1st':'2nd'} Khaiwal chart saved.`);
  }
  ...
  if(section==='khaiwalCharts')return <div className="admin-shell"><header className="admin-top"><strong>Fast Satta Result Admin — MONGO_URI only</strong><button className="primary-button" onClick={logout}>Logout</button></header><div className="admin-grid"><aside className="admin-nav">{Object.entries(sections).map(([key,item])=><button key={key} className={section===key?'active':''} onClick={()=>{setSection(key);setEditing(null);setMessage('')}}>{item.label}</button>)}</aside><main className="admin-main"><h1>Khaiwal Charts</h1><p>Choose either box to independently change its homepage name and WhatsApp number.</p><div className="khaiwal-admin-grid"><div className="admin-card"><h2>1st Khaiwal Chart</h2><form className="admin-form" onSubmit={event=>saveKhaiwal(event,'khaiwal1')}><input name="khaiwalName" placeholder="Khaiwal name" defaultValue={data.settings?.khaiwal_name||''} required/><input name="whatsappNumbers" type="tel" placeholder="WhatsApp number with country code" defaultValue={data.settings?.whatsapp_number||''} required/><button type="submit">Save 1st Khaiwal Chart</button></form></div><div className="admin-card"><h2>2nd Khaiwal Chart</h2><form className="admin-form" onSubmit={event=>saveKhaiwal(event,'otherCharts')}><input name="khaiwalName" placeholder="Khaiwal name" defaultValue={data.otherCharts?.[0]?.khaiwalName||''} required/><input name="whatsappNumbers" type="tel" placeholder="WhatsApp number with country code" defaultValue={data.otherCharts?.[0]?.whatsappNumbers||''} required/><button type="submit">Save 2nd Khaiwal Chart</button></form></div></div>{message&&<div className="admin-card"><strong>{message}</strong></div>}</main></div></div>;
