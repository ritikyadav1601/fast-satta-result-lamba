...
const resultFor = (results, game, date) => {
  const value = results.find(
    (r) => String(r.gameId) === String(game.id) && r.date === date
  )?.result;
...
const gameTime = (value) => {
  ...
  const hour = Number(match[1]),
    minute = match[2];
  return `${String(hour % 12 || 12).padStart(2, "0")}:${minute} ${
    hour < 12 ? "AM" : "PM"
  }`;
};
const minutes = (value) => {
...
const byResultTime = (a, b) =>
  minutes(a.resultTime || a.result_time || a.time) -
  minutes(b.resultTime || b.result_time || b.time);
const orderedGames = (games) => [...games].sort(byResultTime);
const isDesawar = (game) =>
  /desa[wv]ar/i.test(game.englishName || game.english_name || game.name);
const isSecondaryUpper = (game) =>
  /^(prem nagar|jaamu city)$/i.test(
...
const monthlyChartName = (game) =>
  String(game.englishName || game.english_name || game.name || "")
    .trim()
    .toLowerCase()
    .replace(/[^a-z]+/g, " ")
    .trim()
    .replace("delhi bazaar", "delhi bazar")
    .replace("shri ganesh", "shree ganesh")
    .replace("gaziabad", "ghaziabad")
    .replace(/desa[wv]ar/, "desawar");
const hasResult = (results, game, date) =>
  results.some(
    (row) =>
...
const todayResultDisplay = (results, game) =>
  hasResult(results, game, today()) ? (
    resultFor(results, game, today())
  ) : (
    <WaitResult />
  );
const circleGames = (games, results, date) => {
  ...
  const start = lastDeclared
    ? eligible.findIndex(
        (game) => String(game.id) === String(lastDeclared.id)
      ) + 1
    : 0;
  let waiting = eligible
    .slice(start)
    .filter((game) => !hasResult(results, game, date))
    .slice(0, 2);
  if (waiting.length < 2)
    waiting = [
      ...waiting,
      ...eligible
        ...
        .filter(
          (game) =>
            !hasResult(results, game, date) &&
            !waiting.some((item) => String(item.id) === String(game.id))
        )
        .slice(0, 2 - waiting.length),
    ];
  return lastDeclared ? [lastDeclared, ...waiting] : waiting;
};
const khaiwalGames = [
  ["जयपुर मटका", "12:45 PM"],
  ["सदर बाजार", "1:25 PM"],
...
const KhaiwalGameTimings = () =>
  khaiwalGames.map(([name, time]) => (
    <p key={name}>
      <strong>
        ⏰ {name} ------------ {time}
      </strong>
...
export function Hero({ settings, games, results }) {
  const now = new Date(),
    yesterday = new Date(now.getTime() - 86400000),
    disawar = games.find(isDesawar);
  const topGames = circleGames(games, results, today());
  const yesterdayResult = disawar
    ? resultFor(results, disawar, dateKey(yesterday))
    : "--";
  return (
    <>
      ...
      {topGames.length > 0 && (
        <section className="circlebox2">
          {topGames.map((game, index) => {
            const declared = index === 0 && hasResult(results, game, today());
            return (
              <div
                className={
                  declared ? "circle-game-declared" : "circle-game-waiting"
                }
                key={game.id}
              >
          ...
          <div className="container">
            ...
            >
              ...
              <strong style={{ fontSize: 20, letterSpacing: 2 }}>
                ...
                />{" "}
                {disawar ? (
                  todayResultDisplay(results, disawar)
                ) : (
                  <WaitResult />
                )}
              </strong>
...
function TimingCard({ settings }) {
  const name = (settings.khaiwal_name || "").toUpperCase(),
    number = String(settings.whatsapp_number || "").trim(),
    whatsappNumber = number.replace(/\D/g, "");
  return (
    <div className="column-ad">
      <div className="card-body promo-gradient">
        <p>
          <strong>--सीधे सट्टा कंपनी का No 1 खाईवाल--</strong>
        </p>
        <p>
          <strong>♕♕&nbsp; {name} &nbsp;BHAI&nbsp;KHAIWAL ♕♕</strong>
        </p>
        <KhaiwalGameTimings />
  ...
  return (
    <>
      ...
      {games.length > 0 && (
        <>
          {[
            sorted.filter((g) => !isDesawar(g) && !isSecondaryUpper(g)),
            sorted.filter(isSecondaryUpper),
          ].map((group, index) =>
            group.length ? (
              <section
                className="tablebox1"
                      ...
                      {group.map((g) => (
                        <tr key={g.id}>
                          <td className="foryellow">
                            <Link className="gamenameeach" href={chartHref(g)}>
                              <strong>{g.name}</strong>
                            </Link>
                            <br />
                            <strong>
                              {gameTime(
                                g.resultTime || g.result_time || g.time
                              )}
                            </strong>
                          </td>
                          <td className="yesterday-number">
                            <strong>
                              {resultFor(
                                results,
                                g,
                                dateKey(new Date(now.getTime() - 86400000))
                              )}
                            </strong>
                          </td>
                          <td className="today-number">
                            <strong>{todayResultDisplay(results, g)}</strong>
                          </td>
                        </tr>
                      ))}
                    </tbody>
      ...
      <div className="column-ad card-body promo-gradient">
        <p>🙏🏿नमस्कार साथियो 🙏🏿</p>
        <p>
          किसी भी तरह की कोई शिकायत के लिए कंपनी के मैनेजर से संपर्क करे &nbsp;
          &nbsp;
        </p>
        ...
        <p>
          किसी भी भाई को किसी भी तरह की कोई शिकायत या परेशानी हो तो हमसे
          telegram पर संपर्क करे
        </p>
        <p>
      ...
      <section className="octoberresultchart">
        <div className="result-chart-title">
          Fast Satta Result – Live {now.getFullYear()} Updates
        </div>
      </section>
      <section className="octoberresultchart">
        <h2>
      ...
      <section className="newtable">
        <div className="table-responsive marginBottom">
          <table className="table table-bordered table-extra">
            <thead>
              <tr>
                <td className="table_chart_section_01 forfirtcolor date text-center">
              ...
              {days.map((day) => (
                <tr key={day.date}>
                  <td className="forfirtcolor text-center">
                    <span className="fon">{day.label}</span>
                  </td>
                  {chartGames.map((g) => (
                    <td key={g.id}>
                      <span className="table_chart_section_02">
                        {day.date === today() &&
                        !hasResult(results, g, day.date) ? (
                          <WaitResult />
                        ) : (
                          resultFor(results, g, day.date)
                        )}
                      </span>
                    </td>
                  ))}
