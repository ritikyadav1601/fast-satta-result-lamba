...
const options = { appName:'fast-satta-result-extra-games', serverSelectionTimeoutMS:20000, connectTimeoutMS:20000, socketTimeoutMS:30000, maxPoolSize:10, maxIdleTimeMS:60000, retryReads:true };

async function extraDatabase() {
  if (!uri) return null;
  if (!state.promise) {
    state.promise = connectMongo(uri, options).catch(error => { state.promise=null; throw error; });
  }
