...
async function mainDatabase(){
  if(!uri)return null;
  if(!state.promise){state.promise=connectMongo(uri,{appName:'fast-satta-result-main-games',serverSelectionTimeoutMS:6000,connectTimeoutMS:6000,socketTimeoutMS:30000,maxPoolSize:10,maxIdleTimeMS:60000,retryReads:true}).catch(error=>{state.promise=null;throw error})}
