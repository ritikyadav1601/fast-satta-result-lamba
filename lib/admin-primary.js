...
export async function adminPrimaryData(date) {
  ...
  const [games, results, otherCharts, blogs, settings] = await Promise.all([
    ...
    // List view only needs light fields; base64 images/HTML (MBs) made this call time out. Full post is fetched on Edit.
    db.collection('blogs').find({}, { projection: { _id: 0, id: 1, slug: 1, title: 1, shortDescription: 1, published: 1, isPublished: 1, createdAt: 1 } }).sort({ createdAt: -1 }).toArray(),
...
export async function adminBlog(id) {
  const db = await database();
  if (!db) throw new Error('Primary MongoDB is not configured.');
  const values = [id];
  if (/^\d+$/.test(String(id))) values.push(Number(id));
  const blog = await db.collection('blogs').findOne({ id: { $in: values } }, { projection: { _id: 0 } });
  if (!blog) throw new Error('Blog not found.');
  return blog;
}

async function deleteResultFor(allowedIds, item) {
  const gameId = Number(item?.gameId);
  const date = String(item?.date || '');
  if (!allowedIds.includes(gameId) || !/^\d{4}-\d{2}-\d{2}$/.test(date)) throw new Error('Invalid game or date.');
  const db = await database();
  if (!db) throw new Error('Primary MongoDB is not configured.');
  const result = await db.collection('results').deleteMany({ gameId: { $in: [gameId, String(gameId)] }, date });
  if (!result.deletedCount) throw new Error('Result not found.');
  return { deleted: true, gameId, date };
}

export const deleteAdminResult = item => deleteResultFor(ADMIN_GAME_IDS, item);
export const deleteRestrictedAdminResult = item => deleteResultFor(RESTRICTED_ADMIN_GAME_IDS, item);
